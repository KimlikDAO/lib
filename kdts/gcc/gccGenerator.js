...
/**
 * Rewrites readonly array-like type nodes in-place so downstream rendering can
 * use the normal TSTypeReference path for ReadonlyArray<T>.
 *
 * @param {object|undefined} n
 * @return {object|undefined}
 */
const mutateReadonlyArrayLike = (n) => {
  if (!n) return n;

  let elementType;
  if (n.type == "TSArrayType")
    elementType = n.elementType;
  else if (n.type == "TSTupleType")
    elementType = n.elementTypes?.[0];
  else
    return n;

  delete n.elementType;
  delete n.elementTypes;
  n.type = "TSTypeReference";
  n.typeName = { type: "Identifier", name: "ReadonlyArray" };
  n.typeArguments = {
...
class Generator {
  ...
  // TS Type expressions
  TSArrayType(n) { this.put("!Array<"); this.rec(n.elementType); this.put(">"); }
  TSAsExpression(n) {
  ...
  TSTupleType(n) { this.put("!Array<"); this.rec(n.elementTypes[0]); this.put(">"); } // TODO() throw if multiple types
  TSUnionType(n) { this.arr(n.types, "|"); }
  TSQualifiedName(n) {
    if (n.left.type == "Identifier") this.TSIdentifier(n.left); else this.rec(n.left);
    this.put("." + n.right.name);
  }
  ...
  TSTypeOperator(n) {
    if (n.operator == "readonly") {
      mutateReadonlyArrayLike(n.typeAnnotation);
      this.rec(n.typeAnnotation);
      return;
    }
    this.put(n.operator + " "); this.rec(n.typeAnnotation);
  }
  ...
  TSIdentifier(n) { this.put(NameMap[n.name] ?? n.name); }
  TSTypeReference(n) {
    this.put("!");
    if (n.typeName.type == "Identifier") this.TSIdentifier(n.typeName); else this.rec(n.typeName);
    this.rec(n.typeArguments);
  }
  ...
  TSPropertySignature(n, inInterface) {
    if (inInterface) {
      ...
      this.rec(n.typeAnnotation);
      if (n.optional) this.put("|undefined)");
    }
  }
  ...
  Identifier(n, showTypes) {
    if (showTypes == IdentifierTypes.Ts && n.typeAnnotation) {
      ...
      this.put("/** @type {"); this.rec(n.typeAnnotation); this.put("} */ ");
      this.put(this.identifierName(n));
    } else if (showTypes == IdentifierTypes.Param)
      this.rec(n.typeAnnotation);
    else
      this.put(this.identifierName(n));
  }
