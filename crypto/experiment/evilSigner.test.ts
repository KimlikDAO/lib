...
it("generates RFC6979 compliant signatures on all 248 bit inputs",
  async () => {
    for (let d = 0; d < 100; ++d) {
      for (let privKey = 0; privKey < 10; ++privKey) {
        const privKeyBytes = new Uint8Array(32);
        privKeyBytes.set(crypto.getRandomValues(new Uint8Array(31) as Uint8Array), 1);
        const privKey = bigints.fromBytesBE(privKeyBytes);
        const digest = new Uint8Array(32);
        digest.set(crypto.getRandomValues(new Uint8Array(31) as Uint8Array), 1);
