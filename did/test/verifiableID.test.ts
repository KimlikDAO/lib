...
test("generate and verify", () => {
  const secretKey = "MIIBVQIBADANBgkqhkiG9w0BAQEFAASCAT8wggE7AgEAAkEA5gWLDHtfFthoCtHxRTFPCi"
  ...
  return prepareGenerateKey(secretKey)
    .then((generateKey: CryptoKey) =>
      generate("TR22345678902", generateKey))
    .then((verifiableID: VerifiableID) =>
      verify(verifiableID, "TR22345678902", publicKey))
    .then((result: boolean) => expect(result).toBeTrue());
}, {
  timeout: 20_000
});

test("generate and verify", () => {
  const secretKey = "MIIBVQIBADANBgkqhkiG9w0BAQEFAASCAT8wggE7AgEAAkEAxxht6rOMPXdVuvhBvPLuPC"
