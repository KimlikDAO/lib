...
describe("fromWide() and toWide()", () => {
  it("roundtrips wide -> compact -> wide", () => {
    expect(signature.toWide(signature.fromWide(WIDE_SIG))).toBe(WIDE_SIG);
  });

  it("roundtrips compact -> wide -> compact", () => {
    expect(signature.fromWide(signature.toWide(COMPACT_SIG))).toBe(COMPACT_SIG);
  });
});

describe("fromUnpacked()", () => {
  it("packs r, s, yParity into EIP2098 compact signature", () => {
    const r = BigInt("0x278f49cb66db8b104751fe3413dbf50e58288e66b625fa704b33255d06012295");
    const s = BigInt("0x6dc164431e9b78805da4bb590d2a8f7a340c89ed74ec04d054a3b49977ec6b4a");
